...
- Integration
- Improve document
- Consider case when proxy response status code is 500
	as a warning of no network connection
- SMng.sessions is not storing sessions, since (I think)
	is not a concurrent map.
- Change QAdm.canReq to use an ip instead of an URL as
	parameter
